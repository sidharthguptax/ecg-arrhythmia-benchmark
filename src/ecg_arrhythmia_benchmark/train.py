...
def train(model_name):
    ...
    print(
        f"Model: {model_name} | Params: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}"
    )
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        print(
            f"Epoch {epoch:02d} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Train F1: {train_f1:.4f} | Val F1: {val_f1:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, required=True, choices=["cnn", "lstm", "transformer"]
    )
